...
## Storefront async action feedback — 2026-09-05

- Approved design: `docs/superpowers/specs/2026-09-05-storefront-async-action-feedback-design.md`.
- Approved executable plan: `docs/superpowers/plans/2026-09-05-storefront-async-action-feedback.md`.
- Immutable implementation baseline: `ed58d02` (`docs: plan storefront async action feedback`).
- Delivery branch: `fix/storefront-polish`; protected Phase 2 plan files and unrelated untracked hero Playwright config preserved.
- Scope: customer storefront only; use `FadeArc`; no admin, demo-admin, publication, merge, deployment, provider, database, or full-gate operation.
- Task 1: complete (`ed58d02..28d21cf`). Commits `4dcafd6` and `28d21cf`; focused primitive tests 3/3, Prettier, diff check passed. Fresh Sol Medium review: Spec Compliant, Critical 0 / Important 0 / Minor 0 after `asChild` remediation.
- Task 2: complete (`28d21cf..0f52992`). Commits `e600ff9` and `0f52992`; focused auth/gate/primitive tests 21/21, Prettier, ESLint (0 errors; six pre-existing warnings), diff check passed. Fresh Sol Medium review: Compliant, Critical 0 / Important 0 / Minor 0 after Google busy-scope remediation. Task 3 next.
- Task 3: complete (`0f52992..a2c1c92`). Commits `b08727a` and `a2c1c92`; focused catalog/PDP tests 22/22, Prettier, diff check passed. Fresh Sol Medium review: Complete, Critical 0 / Important 0 / Minor 0 after 360 spinner sizing/alignment remediation. Task 4 next.
- Task 4: complete (`a2c1c92..782f707`). Commits `e631aa4`, `874d84c`, and `782f707`; focused cart/catalog tests 40/40, Prettier, diff check passed. Fresh Sol Medium review: Complete, Critical 0 / Important 0 / Minor 0 after quantity-state, rejection, and label/FadeArc test remediation. Typecheck remains blocked only by pre-existing implicit-`any` errors in `tests/evironn-storefront-shell.test.tsx:89-91`.
- Task 5: complete (`782f707..6387f89`). Commits `c3e1306` and `6387f89`; focused checkout/order/cancellation tests 55/55, Prettier, diff check passed. Fresh Sol Medium review: Approved, Critical 0 / Important 0 / Minor 0 after exact FadeArc-marker and rendered cancellation rejection-test remediation. Typecheck remains blocked only by pre-existing implicit-`any` errors in `tests/evironn-storefront-shell.test.tsx:89-91`.
- Task 6: complete (`6387f89..be24831`). Commits `3ba78bf`, `a1f3145`, and `be24831`; focused profile/catalog/primitive tests 38/38, final bounded checkpoint 143/143, Prettier, ESLint, and diff check passed. Fresh Sol Medium review: Approved, Critical 0 / Important 0 / Minor 0 after deferred per-operation and sibling-isolation test remediation. Typecheck parity confirmed against baseline: same three pre-existing `this` errors in `tests/evironn-storefront-shell.test.tsx:89-91`; documented exception, not task regression.
- Final cumulative remediation: commits `9b38710` and `9dd1d27`; ReviewForm now restores pending state on rejection, and profile favorite removal uses one optimistic pending owner with rollback plus preserved accessible label. Fresh final Sol Medium review: PASS, Critical 0 / Important 0 / Minor 0. Latest focused checkpoint reported 144/144; typecheck parity remains the documented three-error baseline exception.
- Independent final verification on `9dd1d27`: exact 11-file checkpoint 144/144; Prettier clean; ESLint 0 errors/13 existing warnings; `git diff --check` clean. `npm run typecheck` still reports only the three unchanged `tests/evironn-storefront-shell.test.tsx:89-91` baseline errors. No full gate/build/E2E, push, PR, merge, deploy, or visual acceptance performed.
- Follow-up bugfix: cart checkout links now show `FadeArc`, retain the label, guard duplicate activation, and navigate through `router.push`; checkout contact/address/promo controls use explicit stable IDs to prevent the reported hydration mismatch. RED/GREEN focused regression tests added; cart+checkout/source contracts 84/84 and final storefront checkpoint 146/146 passed. Typecheck remains limited to the same three pre-existing storefront-shell errors.
- Review remediation `43c7700`: removed the extra cart checkout spinner keyframes so the shared `FadeArc` animation and reduced-motion behavior remain the sole source of spinner motion. Affected focused tests remain green (79/79).
- Runtime follow-up: reproduced the reported cart-item DELETE 500 on the stale `3001` Next dev process as a webpack async-module load failure before route execution. A fresh local dev process returned the expected owner-scoped `401` for the same request and `/cart` returned `200`; no production source change was needed. Focused cart route/UI tests passed 40/40.
- Wishlist follow-up: reproduced guest catalog add failures as a concurrent `wishlist.create()` token collision (`P2002`) in `resolveOwnerWishlist`, which surfaced as a `POST /catalog 500`. First remediation added an owner re-read; durable fix `56c140b` now uses atomic `wishlist.upsert` by unique token. Database inspection then confirmed the logged-in admin had no user wishlist while guest rows existed; fix `9a597ec` recovers that authenticated token collision by claiming the unowned guest wishlist for the admin user, with RED/GREEN coverage in `tests/wishlist.test.ts`. Wishlist/catalog suites pass 46/46; after restarting the dev process, a fresh Chromium click on Terra Chair Ivory settles to `aria-label="Убрать Terra Chair Ivory из избранного"` with no console errors. Typecheck remains limited to the three pre-existing storefront-shell errors.
- Authenticated follow-up: the newly created account reused a wishlist token already owned by another user, so claiming was correctly refused and the action still returned no owner. Final fix `aa69fef` creates a fresh user token on that conflict and persists the replacement cookie; added resolver and action-cookie regression tests. Focused wishlist/catalog suites pass 48/48.
- Dev runtime cleanup: reproduced cart DELETE failures as corrupted generated Next chunks (`Cannot find module './vendor-chunks/@auth.js'`), moved only `.next` to a disposable backup, removed the backup, and restarted `3001`. Fresh `/api/cart/foo` now returns expected `401` (not `500`); dynamic cart route compiles successfully.
