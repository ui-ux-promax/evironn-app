...
- ADR-019 portfolio-only E2E policy approved 2026-08-18; empty forbidden-fingerprint policy is valid, explicit approved-dev URL/write opt-in remain required, and `.env.local` is loaded at standalone E2E boundaries. Policy commit `bca152e`, loader commit `51f75ef`, and Windows Prisma CLI fix `7698051` are committed.
- Portfolio readiness: sanitized completion readiness passed against shared Neon `dev` with all identity/connectivity/migration checks true; guarded Prisma deploy applied all three Phase 4 migrations. No reset/truncate/global delete/schema reset. Full gate and browser E2E remain pending.
...
## Phase 4 checkpoint — paused 2026-08-19

- Branch: `phase/04-checkout-orders`; no push, PR, merge, or branch deletion. Protected pre-existing untracked plans remain untouched: `docs/superpowers/plans/2026-08-12-phase-2a-executable-storefront-home.md` and `docs/superpowers/plans/phase-2-task-3-execution.md`.
- Portfolio-only E2E policy is implemented: shared approved Neon `dev` target, explicit `E2E_DATABASE_URL`/`E2E_DATABASE_URL_UNPOOLED`, `E2E_DATABASE_ALLOW_WRITES=1`, approved fingerprint, empty forbidden list allowed by recorded ADR-019, `.env.local` loading, guarded Windows Prisma CLI, targeted namespace cleanup only.
- Database readiness is green after guarded deploy of 3 Phase 4 migrations: `20260816_phase4_delivery_snapshots`, `20260816_phase4_payment_replay`, `20260817_phase4_payment_claim`. No reset, truncate, global delete, or Production DB used.
- Completion evidence already green: `npm run format`; lint (0 errors, 63 warnings); typecheck; full Vitest `194 files / 1242 tests`; build. Build has existing Sentry-without-token, Tailwind ambiguous-class, and Edge Runtime warnings.
- Critical E2E was first run as 13 scenarios and exposed stale Playwright contracts, not database-target failure. Fixes cover exact password textboxes, same-origin callback assertion, exact address/city fields, showroom case-insensitive label, visible promo field/button/status, Neon read-after-write polling, and stale source-contract expectations. The latest focused browser runs passed checkout `5`, order `3`, and review `2` scenarios.
