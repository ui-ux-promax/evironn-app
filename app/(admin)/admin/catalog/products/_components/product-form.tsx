    ...
    <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="space-y-[22px]">
      ...
      <Field label="Описание" error={errors.description?.message}>
        <textarea
          ...
          rows={4}
          className="w-full rounded-[18px] border border-admin-outline-variant bg-admin-surface px-4 py-3 text-sm outline-none transition-colors focus:border-admin-on-surface"
        />
      </Field>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button type="button" variant="outline" size="sm" onClick={addColorway}>
            Добавить расцветку
          </Button>
        </div>
        ...
        {typeof errors.colorways?.message === 'string' && (
          <p className="text-sm text-admin-error">{errors.colorways.message}</p>
        )}
        {typeof errors.active?.message === 'string' && (
          <p className="text-sm text-admin-error">{errors.active.message}</p>
        )}
      </div>
