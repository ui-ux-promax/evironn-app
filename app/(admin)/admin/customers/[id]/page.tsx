  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <h2 className="font-admin-head text-3xl font-bold text-admin-on-surface">{user.name?.trim() || 'Без имени'}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Section
            title={`История заказов${orderCount > HISTORY_LIMIT ? ` (последние ${HISTORY_LIMIT} из ${orderCount})` : ''}`}
          >
            {orders.length === 0 ? (
                    ...
                    {orders.map((o) => {
                      ...
                      return (
                        <tr key={o.id} className="hover:bg-admin-surface-high transition-colors">
                          ...
                          <td className="px-2 py-3 text-admin-on-surface-variant tabular-nums">
                            {formatDateTime(o.createdAt)}
                          </td>
                          <td className="px-2 py-3">
                            <span className={sv.badge}>{sv.label}</span>
                          </td>
                          <td className="px-2 py-3 text-right font-bold text-admin-on-surface tabular-nums">
