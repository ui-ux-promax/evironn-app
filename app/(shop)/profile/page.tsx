...
export default async function ProfilePage() {
  ...
  const orders: ProfileOrder[] = user.orders.map((order) => ({
    ...
    items: order.items.map((item) => ({
      productNameFromVariant: item.productVariant?.colorway.product.name ?? item.productName,
      brand: item.productVariant?.colorway.product.brand ?? 'Evironn',
      colorwayName: item.colorwayName || item.productVariant?.colorway.name || '—',
      size: item.size || item.productVariant?.size || '—',
      price: item.unitPrice || item.productVariant?.price || 0,
