...
export function AdminPanel({ title, note, actions, children, className }: AdminPanelProps) {
  return (
    <section
      className={cn(
        'rounded-2xl border border-admin-outline-variant bg-admin-surface p-[22px] shadow-[var(--admin-shadow-tight)]',
        className,
      )}
    >
      {(title || note || actions) && (
        <div className="mb-[18px] flex flex-wrap items-start justify-between gap-3.5">
          <div>
            ...
            {note && <p className="mt-[5px] max-w-[72ch] text-xs text-admin-on-surface-variant">{note}</p>}
          </div>
