...
const Table = React.forwardRef<HTMLTableElement, React.HTMLAttributes<HTMLTableElement>>(
  ({ className, ...props }, ref) => (
    <div className="relative w-full overflow-auto">
      <table ref={ref} className={cn('w-full border-collapse text-[13px] caption-bottom', className)} {...props} />
    </div>
  ),
);
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        'h-10 px-3.5 pb-2.5 text-left align-middle text-[10px] font-bold uppercase tracking-[.12em] text-admin-on-surface-variant [&:has([role=checkbox])]:pr-0',
        className,
      )}
      {...props}
    />
...
const TableCell = React.forwardRef<HTMLTableCellElement, React.TdHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <td
      ref={ref}
      className={cn('px-3.5 py-3.5 align-middle text-admin-on-surface [&:has([role=checkbox])]:pr-0', className)}
      {...props}
    />
  ),
);
