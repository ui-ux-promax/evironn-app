...
/**
 * Swipe-to-dismiss for the mobile sheet.
 *
 ...
 * suppresses the click that would otherwise follow a drag released over a link.
 */
export function useSheetDrag({ sheetRef, scrimRef, open, onDismiss }: SheetDragOptions) {
  useEffect(() => {
    ...
    const paint = (value: number) => {
      ...
      if (scrim) scrim.style.opacity = String(Math.max(0, 1 - value / Math.max(1, sheet.offsetHeight)));
    };
