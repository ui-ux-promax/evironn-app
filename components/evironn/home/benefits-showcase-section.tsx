...
function Icon({ name }: { name: IconName }) {
  ...
  return <IconComponent aria-hidden="true" className={`benefit-icon benefit-icon-${name}`} strokeWidth={1.8} />;
}

function RevealWord({ word, order }: { word: string; order: number }) {
  const reduceMotion = useReducedMotion();
  const animation = useEditorialAnimation(undefined, order * 0.12);
  return (
    <motion.span
...
function FooterRevealMedia({
  ...
  return (
    <motion.div
      className={className}
      initial={reduceMotion ? false : { opacity: 0, y: 40, filter: 'blur(12px)', rotate }}
...
export function BenefitsShowcaseSection() {
  return (
    <section aria-labelledby="benefits-showcase-title-primary" className="benefits-showcase">
      <div className="benefits-showcase-container">
        <header className="benefits-showcase-header">
          <div className="benefits-showcase-title">
            ...
            />
            <RevealLine className="benefits-showcase-title-secondary" orderStart={3} words={['не', 'для', 'витрины']} />
          </div>
        ...
        <div className="benefits-showcase-grid">
          <article className="benefit-story">
            ...
            <RevealMedia className="benefit-story-quote" initial={{ opacity: 0, y: 30 }}>
              <Icon name="hammer" />
              <div className="benefit-story-quote-copy">
                <h3>Точная работа с материалом</h3>
                <p>Каждая линия и соединение остаются на виду, чтобы мебель красиво служила годами.</p>
              </div>
            </RevealMedia>
          </article>
            ...
            <article className="benefit-materials">
              <div className="benefit-materials-copy">
                ...
                <div>
                  ...
                  <p>Удобные детали, спокойные формы и материалы, которые приятно трогать каждый день.</p>
                </div>
              </div>
              <RevealMedia
