...
import {
  ...
  getCaptionOrder,
} from './hero-product-motion';
import type { HeroProduct } from './hero-products';
...
export function HeroProductCaption({ product }: HeroProductCaptionProps) {
  ...
  const { nameWords, categoryOrder, priceOrder } = getCaptionOrder(product.name);
    ...
    <>
      <span className="furni-hero-product__eyebrow">
        <motion.span
          ...
          initial="hidden"
          animate="visible"
          variants={createFurnitureCaptionVariants(categoryOrder, baseDelay, reducedMotion)}
        >
          {product.category}
        </motion.span>
      </span>
        ...
        {nameWords.map((word, index) => (
          <Fragment key={`${word}-${index}`}>
            ...
            <motion.span
              ...
              initial="hidden"
              animate="visible"
              variants={createFurnitureCaptionVariants(index, baseDelay, reducedMotion)}
            >
              {word}
            </motion.span>
          </Fragment>
      ...
      <motion.span
        ...
        initial="hidden"
        animate="visible"
        variants={createFurnitureCaptionVariants(priceOrder, baseDelay, reducedMotion)}
      >
        {product.price}
      </motion.span>
      {product.href ? (
        <MotionLink
          ...
          initial="hidden"
          animate="visible"
          variants={createFurnitureCaptionVariants(linkOrder, baseDelay, reducedMotion)}
        >
