...
export type HeroPhase = 'idle' | HeroProductId | `entering-${HeroProductId}` | `returning-${HeroProductId}`;
...
export function getHeroProduct(phase: HeroPhase): HeroProductId | null {
  return (
    HERO_PRODUCT_IDS.find((id) => phase === id || phase === `entering-${id}` || phase === `returning-${id}`) ?? null
  );
}
...
export function selectHeroProduct(phase: HeroPhase, product: HeroProductId): HeroPhase {
...
export function cancelHeroProduct(_phase: HeroPhase): HeroPhase {
  return 'idle';
}
...
export function shouldRevealHeroProduct(currentTime: number, duration: number) {
  ...
  return Number.isFinite(duration) && duration > 0 && progress + 1e-6 >= HERO_CARD_REVEAL_PROGRESS;
}
