...
export const AVAILABLE_HERO_ROOM_IDS = ['living-room', 'kitchen', 'bedroom', 'terrace'] as const;
...
export function isAvailableHeroRoom(room: HeroRoomId): room is AvailableHeroRoomId {
...
export function requestHeroRoom(
  ...
  if (state.phase !== 'idle' || !targetReady || !isAvailableHeroRoom(target) || target === state.activeRoom) {
    return state;
  }
