...
export function Hero() {
  ...
  const [stackRoom, setStackRoom] = useState<AvailableHeroRoomId>(INITIAL_HERO_ROOM_STATE.activeRoom);
  const [stackFade, setStackFade] = useState<'idle' | 'out' | 'in'>('idle');
  const [readyRooms, setReadyRooms] = useState<Set<AvailableHeroRoomId>>(() => new Set(['living-room']));
  ...
  const beginRoomChange = useCallback(
    (target: AvailableHeroRoomId, direct = false) => {
      ...
      setRoomState((current) => requestHeroRoom(current, target, readyRooms.has(target), direct));
    },
    [readyRooms],
  );

  const handleRoomClick = (room: HeroRoomId) => {
    if (!isAvailableHeroRoom(room) || roomChanging || room === selectedRoom || !readyRooms.has(room)) {
      return;
    }
      ...
      >
        <article className="furni-hero-panel">
          ...
          <p className="furni-hero-copy">
            Откройте вневременные предметы, созданные нашей семьёй и наполненные теплом традиций и красотой натурального
            дерева.
          </p>
        ...
        <div className="furni-hero-segments">
          <div className="seg-control" ref={segRef} role="group" aria-label="Категория комнаты">
            ...
            {HERO_ROOM_OPTIONS.map((room) => {
              ...
              const roomLocked = !room.available || waitingForMedia || roomChanging;
