...
const standardTransition = { type: 'tween' as const, duration: 0.4, ease: [0.44, 0, 0.56, 1] as const };

function RevealWord({ word, delay }: { word: string; delay: number }) {
  const reduceMotion = useReducedMotion();
  const animation = useEditorialAnimation(undefined, delay);
  return (
    <motion.span
...
function RevealGallery({ children, onReveal }: { children: ReactNode; onReveal: () => void }) {
  const reduceMotion = useReducedMotion();
  return (
    <motion.div
...
function GalleryCard({ source, onHoverChange }: { source: string; onHoverChange: (value: boolean) => void }) {
  return (
    <Link
...
function GalleryTrack({ sources, mobile, active }: { sources: string[]; mobile?: boolean; active: boolean }) {
  const reduceMotion = useReducedMotion();
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      speedRef.current += (targetSpeed - speedRef.current) * Math.min(1, delta * 5);
  ...
  return (
    ...
    >
      {[...sources, ...sources].map((source, index) => (
        <GalleryCard key={`${source}-${index}`} onHoverChange={setHovered} source={source} />
      ))}
    </div>
  );
}
...
export function InstagramFollowSection() {
  ...
  return (
    <section aria-labelledby="instagram-follow-heading" className="instagram-follow">
      <div className="instagram-follow-header">
        <h2 id="instagram-follow-heading">
          ...
          <span aria-hidden="true" className="instagram-follow-floating-stack">
            <FloatingPhoto rotate={-10} source="/assets/products/03-ivory-lounge-idle.webp" />
            <FloatingPhoto rotate={10} source="/assets/products/05-two-seat-sofa-idle.webp" />
          </span>{' '}
      ...
      <RevealGallery onReveal={() => setIsGalleryRevealed(true)}>
        <div className="instagram-follow-gallery-viewport">
          ...
          <GalleryTrack active={isGalleryRevealed} mobile sources={mobileGalleryImages} />
        </div>
      </RevealGallery>
    </section>
  );
