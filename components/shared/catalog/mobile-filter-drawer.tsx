  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger className="btn btn-sm btn-secondary md:hidden" aria-label="Открыть фильтры">
        ...
        Фильтры
        {activeCount > 0 && (
          <span className="ml-1 text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-primary text-primary-foreground tnum">
            {activeCount}
          </span>
        )}
      </Dialog.Trigger>
      <Dialog.Portal>
        ...
        >
          <div className="flex h-16 shrink-0 items-center justify-between border-b border-line px-5">
            ...
            <Dialog.Close
              className="w-9 h-9 grid place-items-center rounded-full hover:bg-surface-soft"
