...
const links: NavLink[] = [
  { label: 'Новинки', href: '/catalog?sort=new', match: (p, s) => p === '/catalog' && s.get('sort') === 'new' },
  {
    ...
    match: (p, s) => p === '/catalog' && s.get('category') === 'tees',
  },
  {
    label: 'Худи',
    href: '/catalog?category=hoodies',
    match: (p, s) => p === '/catalog' && s.get('category') === 'hoodies',
  },
  {
    label: 'Верхняя одежда',
    href: '/catalog?category=outerwear',
    match: (p, s) => p === '/catalog' && s.get('category') === 'outerwear',
  },
  { label: 'Каталог', href: '/catalog', match: (p, s) => p === '/catalog' && !s.get('category') && !s.get('sort') },
];

const NAV_CLASS =
  'hidden md:inline-flex relative items-center p-1 ml-4 rounded-full bg-surface border border-line shadow-sm isolate';
const ITEM_CLASS =
  'relative z-10 px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors duration-300';
    ...
    <nav className={NAV_CLASS} aria-label="Основная навигация">
      <span
        ...
        className={cn(
          ...
          animate &&
            'transition-[transform,width,opacity] duration-[420ms] ease-[cubic-bezier(.34,1.56,.64,1)] motion-reduce:transition-none',
        )}
        style={{
