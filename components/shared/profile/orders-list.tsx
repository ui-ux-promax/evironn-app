        ...
        <li key={o.orderNumber} className="rounded-2xl border border-line bg-surface p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <Link href={`/orders/${o.orderNumber}`} className="font-semibold hover:underline">
                Заказ #{o.orderNumber}
              </Link>
            ...
            <div className="flex items-center gap-2 mt-3">
              {o.thumbs.map((t, i) => (
                <div key={i} className="relative w-16 h-16 rounded-xl overflow-hidden bg-surface-soft shrink-0">
                  {t.imageUrl && (
                    <Image src={t.imageUrl} alt={t.productName} fill sizes="64px" className="object-cover" />
                  )}
                </div>
              ))}
          ...
          <div className="flex items-center justify-between mt-3">
            <span className={o.status === 'CANCELLED' ? 'tnum line-through text-ink-muted' : 'font-semibold tnum'}>
              {formatPrice(o.totalAmount)}
            </span>
            <Button asChild variant="secondary" size="sm">
              <Link href={`/orders/${o.orderNumber}`}>Подробнее</Link>
            </Button>
          </div>
        </li>
      ))}
