...
export function PersonalDataForm({ initial, email }: { initial: ProfileValues; email: string }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-w-md">
      <div>
        <label htmlFor="p-email" className="label mb-2 block">
          Email
        </label>
        <Input id="p-email" value={email} disabled readOnly />
      </div>
      <div>
        <label htmlFor="p-name" className="label mb-2 block">
          Имя
        </label>
      ...
      <div>
        <label htmlFor="p-phone" className="label mb-2 block">
          Телефон
        </label>
      ...
      <div>
        <label htmlFor="p-bd" className="label mb-2 block">
          Дата рождения
        </label>
