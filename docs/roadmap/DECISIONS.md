...
## ADR-029 — operational commerce funnel on the dashboard

User approved replacing the dashboard's unavailable storefront-style stages with an operational funnel backed only by persisted Evironn data. For each selected period, the five stages are created carts, non-cancelled orders, orders with a succeeded online payment, orders currently SHIPPED or DELIVERED, and orders currently DELIVERED. The delivery stage includes delivered orders because they have passed the delivery handoff; cancelled orders never enter the order or payment counts.

The conversion footer and conversion KPI use `non-cancelled orders / created carts`, rounded to one decimal percentage point. The footer shows a change in percentage points only when both the current and prior equal-length periods contain at least one created cart; otherwise it omits the comparison. COD orders remain orders but never appear as paid without a persisted succeeded Payment. No visit, checkout-start, tracking, schema, provider, or synthetic analytics data is introduced.

## ADR-030 — dual-codec hero transition rollout

The user approved the Terrace sofa VP9 CQ28 experiment on 2026-09-01 after its forward/reverse pair achieved VMAF `98.530034`/`98.609025`, reduced combined bytes by `55.09775429127088%`, and showed no user-visible quality difference. The remaining seven hero transition pairs will therefore reuse the verified pilot recipes rather than repeat codec exploration: VP9 CQ28 WebM is the preferred source and H.264 CRF20 MP4 is the compatibility fallback. A file that misses the unchanged VMAF 95 or stream gate may retry only the already tested VP9 CQ24 or H.264 CRF18 higher-quality setting; unresolved failures block promotion.

All candidates are generated and validated before any production-media replacement. Runtime capability selection loads one format only, preserves the existing poster-first interaction boundary, and may fall back from WebM to MP4 once without a retry loop. The rollout covers only the sixteen `/assets/hero/*-(forward|reverse)` transitions; product-card videos, turntables, focus/idle images, and unrelated delivery behavior remain excluded. No additional manual comparison page is required. Preview deployment and comparable public after-measurement remain separately authorized external operations.
