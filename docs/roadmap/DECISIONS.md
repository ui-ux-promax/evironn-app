...
Each internal delivery ends with durable progress and a handoff but does not merge independently. The sequence is 5A access/shell/dashboard foundation, 5B furniture catalog/SKU/media operations, 5C orders/customers/roles/coupons, and 5D synthetic read-only demo/integration/visual acceptance. Task implementation uses Luna High, task review uses fresh Sol Medium, final functional review uses Sol Medium, and the concrete ADMIN/role/Cloudinary security boundary justifies a separate Sol High security review. All agents use the normal/default service tier.
