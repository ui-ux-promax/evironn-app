...
The pilot covers only approved catalog work from `docs/roadmap/ROADMAP.md` and existing furniture catalog implementation. It must preserve server-authority, normalized SKU, 360-media, and deferred-scope decisions in `docs/roadmap/DECISIONS.md`.

## Required sequence

1. Confirm Phase 1 merge, branch `phase/02-storefront`, and clean starting state.
2. Prepare executable task plan; review plan for conflicts before Task 1.
3. Execute tasks sequentially with focused verification, task review, and remediation loop for Critical/Important findings.
4. Record completed tasks and review results in `.superpowers/sdd/progress.md`.
5. Perform final pilot-only review.
6. Run the Phase 2 quality gate and relevant catalog E2E scenarios.
7. Record review iterations, findings, failed checks, interventions, and validation readings in the final pilot report.
