...
The storefront header is fixed and does not consume document-flow height, so the loading wrapper reserves the full viewport rather than subtracting a header height. The implementation is limited to the cart route loading fallback. The ready cart page, empty-cart state, shared shop layout, and footer styles remain unchanged.

## Behavior

- During cart route loading, the skeleton occupies at least the full viewport beneath the fixed storefront header.
...
- The loading wrapper has a minimum height based on `100svh` with a `100dvh` mobile/browser-UI override and preserves its existing spacing.
