...
const getConnectionString = () => process.env.POSTGRES_URL ?? process.env.POSTGRES_URL_NON_POOLING;
...
const isTransientError = (err: unknown): boolean => {
  ...
  if (typeof e.message === 'string' && TRANSIENT_MESSAGE_FRAGMENTS.some((f) => (e.message as string).includes(f)))
    return true;
