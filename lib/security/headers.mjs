...
const frameSrc = ["'self'", 'https://yoomoney.ru', 'https://*.yookassa.ru'];
...
export function buildContentSecurityPolicy({
  ...
  allowLocalHttpE2e = process.env.E2E_HTTP_LOCAL === 'true',
} = {}) {
  const effectiveScriptSrc = allowVercelLive ? [...scriptSrc, 'https://vercel.live'] : scriptSrc;
  const effectiveConnectSrc = allowVercelLive
  ...
  if (!allowLocalHttpE2e) directives.push('upgrade-insecure-requests');
