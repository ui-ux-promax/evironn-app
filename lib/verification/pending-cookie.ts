...
import { PENDING_VERIFICATION_COOKIE, PENDING_VERIFICATION_MAX_AGE } from '@/constants/config';
...
export function signPending(
  ...
  const payload: PendingPayload = safeCb && safeCb !== '/' ? { email, exp, cb: safeCb } : { email, exp };
