...
import { FurnitureCaption } from './FurnitureCaption';
import {
  CARD_PLAYBACK_RATE,
  getMediaLayerState,
  getReverseStartTime,
...
export function FeaturedProducts({
  ...
  const [videoFrameReady, setVideoFrameReady] = useState(() =>
    FEATURED_PRODUCTS.map(() => false),
  );
  const [fallbackModes, setFallbackModes] = useState<MediaFallbackMode[]>(() =>
    FEATURED_PRODUCTS.map(() => 'idle'),
  );
  ...
  const operationRefs = useRef<number[]>(FEATURED_PRODUCTS.map(() => 0));
  const cardPhases = useRef<CardPhase[]>(FEATURED_PRODUCTS.map(() => 'idle'));
  const mobileState = useRef<MobilePlaybackState>({
    activeIndex: null,
    pendingIndex: null,
    phase: 'idle',
  });
  ...
  const setFrameReady = (index: number, isReady: boolean) => {
    setVideoFrameReady((current) =>
      current.map((value, currentIndex) =>
        currentIndex === index ? isReady : value,
      ),
    );
  };
  ...
  const setMediaLayerVisibility = (
    ...
    media
      ?.querySelector('img')
      ?.classList.toggle('is-hidden', !layerState.showIdleFrame);
    media
      ?.querySelector('canvas')
      ?.classList.toggle('is-visible', layerState.showFrozenFrame);
    setFallbackModes((current) =>
      current.map((value, currentIndex) =>
        currentIndex === index ? fallbackMode : value,
      ),
    );
  };
  ...
  const loadAt = (
    ...
    const effectiveFallback =
      fallbackMode === 'frozen' && freezeCurrentFrame(index)
        ? 'frozen'
        : 'idle';
    video.pause();
  ...
  const restoreIdle = (
    index: number,
    onReady?: () => void,
    resetForwardFrame = false,
  ) => {
    cardPhases.current[index] = 'idle';
    if (resetForwardFrame) {
      loadAt(index, FEATURED_PRODUCTS[index].forwardSrc, 0, false, onReady);
      return;
    }
    onReady?.();
  };
  ...
  const reverseCard = (index: number) => {
    ...
    const reverseTime = getReverseStartTime(
      video.currentTime,
      video.duration,
      video.duration,
    );
    ...
    loadAt(
      index,
      FEATURED_PRODUCTS[index].reverseSrc,
      reverseTime,
      true,
      undefined,
      () => {
  ...
  const activateCard = (index: number, fromMobile = false) => {
    ...
    if (previousIndex === index && cardPhases.current[index] !== 'reverse')
      return;

    if (previousIndex !== null && previousIndex !== index)
      reverseCard(previousIndex);
    ...
    loadAt(
      index,
      FEATURED_PRODUCTS[index].forwardSrc,
      reducedMotion ? Number.MAX_SAFE_INTEGER : 0,
      !reducedMotion,
    );
  };
  ...
  const requestFromTap = (index: number) => {
    ...
    if (nextState.phase === 'reverse' && nextState.activeIndex !== null)
      reverseCard(nextState.activeIndex);
  };
  ...
  return (
    <section
      className="interactive-furniture"
      aria-labelledby="interactive-furniture-title"
    >
