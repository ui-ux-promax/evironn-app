...
export const PRODUCT_SCENE_BACKGROUND =
  '/assets/products/05-graphite-walnut-room-background-fixed.png';
...
export function dragHintForInput({
  isCoarse,
  maxTouchPoints,
}: {
  isCoarse: boolean;
  ...
  return isCoarse || maxTouchPoints > 0
    ? 'Потяни кресло'
    : 'Потяни кресло мышью';
}
