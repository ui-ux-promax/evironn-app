...
const fake = vi.hoisted(() => {
  ...
  const prisma = {
    ...
    cartItem: {
      upsert: async (args: any) => {
        ...
        if (it) {
          applyQ(it, args.update.quantity);
          return { ...it };
        }
      ...
      findMany: async (args: any) => {
        ...
        return state.items
          ...
          .map((i) =>
            args?.include?.productVariant ? { ...i, productVariant: { stock: stockOf(i.productVariantId) } } : { ...i },
          );
      },
    },
  };
  ...
  return {
    ...
    seedItem: (i: { id: string; cartId: string; productVariantId: string; quantity: number }) =>
      state.items.push({ ...i }),
