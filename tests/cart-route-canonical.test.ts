...
describe('canonical cart mutation boundaries', () => {
  ...
  it('rejects legacy productVariantId as a new write contract', async () => {
    ...
    expect(await json(response)).toMatchObject({ code: 'CART_INVALID' });
  ...
  it('rejects missing, inactive, and zero-stock SKU with exact errors', async () => {
    ...
    expect(await json(response)).toMatchObject({ code: 'CART_SKU_NOT_FOUND' });
    ...
    expect(await json(response)).toMatchObject({ code: 'CART_UNAVAILABLE' });
    tx.sku.findUnique.mockResolvedValueOnce({ id: 'sku-1', active: true, stock: 0, product: { active: true } });
    response = await POST(request({ skuId: 'sku-1' }) as never);
    expect(response.status).toBe(409);
    expect(await json(response)).toMatchObject({ code: 'CART_OUT_OF_STOCK' });
  });
  ...
  it('rejects increment beyond stock and quantity maximum', async () => {
    ...
    expect(response.status).toBe(400);
    expect(await json(response)).toMatchObject({ code: 'CART_QUANTITY_LIMIT' });
  });

  it('retries P2002/P2034 by rereading current quantity and stock', async () => {
  ...
  it('returns CART_CONFLICT after bounded retry exhaustion, never generic 500', async () => {
    ...
    expect(response.status).toBe(409);
    expect(await json(response)).toMatchObject({ code: 'CART_CONFLICT' });
  ...
  it('returns exact owner and item errors for scoped item mutations', async () => {
    ...
    expect(await json(response)).toMatchObject({ code: 'CART_OWNER_REQUIRED' });
    ...
    expect(response.status).toBe(404);
    expect(await json(response)).toMatchObject({ code: 'CART_ITEM_NOT_FOUND' });
  });
});
