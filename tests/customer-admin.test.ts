...
describe('customer-admin helpers', () => {
  ...
  describe('roleChangeGuard', () => {
    ...
    it('promote CUSTOMER → ADMIN always allowed', () => {
      expect(roleChangeGuard({ ...base, adminCount: 0, targetRole: 'CUSTOMER', requestedRole: 'ADMIN' }).ok).toBe(true);
    });
    ...
    it('allows demoting another admin when others remain', () => {
      expect(roleChangeGuard({ ...base, adminCount: 2, targetRole: 'ADMIN', requestedRole: 'CUSTOMER' }).ok).toBe(true);
    });
  });
});
