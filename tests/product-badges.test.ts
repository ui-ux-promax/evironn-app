...
import { isNewByDate, discountPercent, stockSummary, computeBadges } from '@/lib/product-badges';
...
describe('stockSummary', () => {
  it('суммирует сток активных вариантов и классифицирует', () => {
    ...
    expect(stockSummary([{ stock: 10, active: true }], 3)).toEqual({ total: 10, soldOut: false, low: false });
  });
  it('неактивные варианты не учитываются', () => {
    expect(stockSummary([{ stock: 99, active: false }])).toEqual({ total: 0, soldOut: true, low: false });
  });
});
